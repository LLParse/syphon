...
if [[ "$mode" = "master" ]]; then
    ...
    docker run --net=host -i elodina/syphon ./marathon_deploy --master zk://master:2181/mesos --docker.registry.url master:5000 --marathon.url http://master:8080 --topics test_src --task.threads 1 --artifacts.host master --artifacts.port 8888 --ssl.cert cert.pem --ssl.key=key.pem --ssl.cacert=server.crt --consumer.config=consumer.properties --target.url=https://10.1.16.228:31002/ --api.key testkey --api.user johnsnow
fi
